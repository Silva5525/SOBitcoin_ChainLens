...
# go to fixtures for tests
cd fixtures/blocks

# unpack
gunzip *.gz

Analyze a raw transaction (hex):

```bash
chainlens_cli tx --hex <raw_tx_hex>
```

Or from file:

```bash
chainlens_cli tx --file tx.hex
```
...
# you find fixtures .json for tests
...
ChainLens **analyzes** them — fast, deterministic, and # ChainLens
