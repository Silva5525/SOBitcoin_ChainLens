...
pub fn analyze_tx(network: &str, raw_tx_hex: &str, prevouts: &[Prevout]) -> Result<TxReport, String> {
    ...
    let mut prevmap: HashMap<(String, u32), PrevoutEntry> = HashMap::new();
    for p in prevouts {
        prevmap.insert(
            (p.txid_hex.to_lowercase(), p.vout),
            PrevoutEntry {
                value_sats: p.value_sats,
                script_pubkey_hex: p.script_pubkey_hex.to_lowercase(),
            },
        );
    }
    ...
    for i in 0..vin_count {
        ...
        let entry = prevmap
            .get(&(txid_in.to_lowercase(), vout_in))
